...
import { FileDown, Truck, ArrowDown, Plus, Package, CalendarIcon, FileSpreadsheet } from "lucide-react";
...
import { format, parse, isWithinInterval, startOfDay, endOfDay, subDays, startOfMonth, endOfMonth } from "date-fns";
...
const Shipping: React.FC = () => {
  ...
  const filteredShipments = shipments.filter((shipment) => {
    ...
    if (dateRange?.from || dateRange?.to) {
      const shipmentDate = parse(shipment.date, "MMM dd, yyyy", new Date());
      ...
      if (startDate && endDate) {
        if (!isWithinInterval(shipmentDate, { start: startDate, end: endDate })) {
          return false;
        }
      } else if (startDate && !isWithinInterval(shipmentDate, { start: startDate, end: startDate })) {
        return false;
      } else if (endDate && !isWithinInterval(shipmentDate, { start: endDate, end: endDate })) {
        return false;
      }
    }
...
Phone: ${defaultAddress.phone}</p>
  ...
  const handleExportExcel = () => {
    try {
      ...
      const excelData = filteredShipments.map(shipment => ({
        ...
        'Date': shipment.date,
        'Courier Service': shipment.courierService,
        'Tracking ID': shipment.trackingId,
        'Status': shipment.status,
        'Weight': shipment.weight || '-',
        'Delivery Cost': shipment.deliveryCost || '-'
      }));
      ...
      toast.success('Shipments exported to Excel successfully');
    } catch (error) {
      console.error('Error exporting to Excel:', error);
      toast.error('Failed to export shipments to Excel');
    }
  };
  ...
  const getDispatchStats = () => {
    ...
    const todayDispatches = shipments.filter(shipment => {
      const shipmentDate = new Date(shipment.date);
      return shipment.status === "dispatched" && 
             isWithinInterval(shipmentDate, { start: startOfDay(today), end: endOfDay(today) });
    });

    const yesterdayDispatches = shipments.filter(shipment => {
      const shipmentDate = new Date(shipment.date);
      return shipment.status === "dispatched" && 
             isWithinInterval(shipmentDate, { start: startOfDay(yesterday), end: endOfDay(yesterday) });
    });

    const monthlyDispatches = shipments.filter(shipment => {
      const shipmentDate = new Date(shipment.date);
      return shipment.status === "dispatched" && 
             isWithinInterval(shipmentDate, { start: startOfCurrentMonth, end: endOfCurrentMonth });
    });

    return {
      today: todayDispatches.length,
      yesterday: yesterdayDispatches.length,
      monthly: monthlyDispatches.length
    };
  };
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{dispatchStats.today}</div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Yesterday's Dispatches</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{dispatchStats.yesterday}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Monthly Dispatches</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{dispatchStats.monthly}</div>
              <p className="text-xs text-muted-foreground">
